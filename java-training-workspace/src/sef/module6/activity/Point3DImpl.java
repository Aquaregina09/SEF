...
public class Point3DImpl extends Point2DImpl implements Point3D{
	...
	double xy = Math.pow((super.getDistance(p)),2);
	double z3 = Math.pow(p.getZ() - this.z, 2);
		return Math.sqrt(z3 + xy);
	}
	...
	public double getDistance(double x2, double y2, double z2){
		double xy = Math.pow(super.getDistance(x2, y2), 2);
		double z3 = Math.pow(z2 - this.z, 2);
		return Math.sqrt(xy + z3);
